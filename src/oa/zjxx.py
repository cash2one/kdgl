    ...
    try:
        ...
        with myapi.connection() as con:
            ...
            while rs.next():
                ...
                return HttpResponse( json.dumps( content) , content_type='application/json' )
        # 新增快递信息
        sql = "insert into  express_info(type, shelf_number, express_status, express_number, express_money, reciver_phone, reciver_address, reciver_name, sender_phone, sender_address, sender_name,express_user_id, express_classification,created_at)values( '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % ( type, shelf_number, express_status, express_number, express_money, reciver_phone, reciver_address, reciver_name, sender_phone, sender_address, sender_name,express_user_id, express_classification,created_at )
        with myapi.connection() as con:
            cur = con.cursor()
            cur.execute(sql)
        log.info( "kdgl", "添加快递信息sql【%s】", sql )
        if cur.rowcount == 1:
            log.info( "add_kdxx", "添加快递信息成功！" )
            # 计算用户绩效
            sql = "select performance from auth_user where number='%s'"%express_user_id
            with myapi.connection() as con:
                cur = con.cursor()
                rs = myapi.sql_execute(cur, sql)
                if rs.next():
            ...
            # 更新用户绩效
            sql = "update auth_user set performance='%s' where number='%s'" % ( performance, express_user_id )
            with myapi.connection() as con:
                cur = con.cursor()
                cur.execute(sql)
            content['xym'] = '000000'
...
def download_view(request):
    ...
    try:
        ...
        if not request.session.get('username',''):
            return HttpResponseRedirect('/')
        title = [u'快递单号', u'快递类型', u'快递分类', u'快递费' , u'快递员编号', u'发货人姓名',
